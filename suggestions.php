<?php // suggestions.php :: Internal forums script for the game.
...
$updatequery = doquery("UPDATE {{table}} SET location='Suggestions Forum' WHERE id='".$userrow["id"]."' LIMIT 1", "users");
...
function donothing($start=0) {
      ...
      $query2 = doquery("SELECT * FROM {{table}} WHERE pin='1' ORDER BY newpostdate DESC LIMIT 30", "suggestions");
 $page = "<table width='100%' border='1'><tr><td class='title'>Suggestions Forum</td></tr></table><p>";
 ...
 $page .= "<hr /><table width=\"100%\"><tr><td style=\"padding:1px; background-color:black;\"><table width=\"100%\" style=\"margins:0px;\" cellspacing=\"1\" cellpadding=\"3\"><tr><th colspan=\"4\" style=\"background-color:#dddddd;\"><center>Only Administrators and Moderators can Pin Threads.</center></th></tr><tr><th width=\"44%\" style=\"background-color:#dddddd;\">Pinned Threads</th><th width=\"2%\" style=\"background-color:#dddddd;\">Replies</th><th  width=\"10%\" style=\"background-color:#dddddd;\">Author</th><th  width=\"30%\" style=\"background-color:#dddddd;\">Last Post</th></tr>\n";
    ...
    if (mysql_num_rows($query2) == 0) {
      ...
      while ($row = mysql_fetch_array($query2)) {
		...
		if ($count == 1) {
                $page .= "<tr><td style=\"background-color:#ffffff;\">".$namelink2."<a href=\"suggestions.php?do=thread:".$row["id"].":0\">".$row["title"]."</a></td><td style=\"background-color:#ffffff;\">".$row["replies"]."</td><td style=\"background-color:#ffffff;\">".$row["author"]."</td><td style=\"background-color:#ffffff;\">".$row["newpostdate"]."</td></tr>\n";
			$count = 2;
		} else {
                $page .= "<tr><td style=\"background-color:#eeeeee;\">".$namelink2."<a href=\"suggestions.php?do=thread:".$row["id"].":0\">".$row["title"]."</a></td><td style=\"background-color:#eeeeee;\">".$row["replies"]."</td><td style=\"background-color:#eeeeee;\">".$row["author"]."</td><td style=\"background-color:#eeeeee;\">".$row["newpostdate"]."</td></tr>\n";
			$count = 1;
		}
	  }
    }
...
$query= doquery("SELECT * FROM {{table}} WHERE parent='0' AND pin!='1' ORDER BY newpostdate DESC LIMIT ".$start.",12", "suggestions");
$fullquery = doquery("SELECT * FROM {{table}} WHERE parent='0' AND pin!='1' ORDER BY newpostdate", "suggestions");
      $page .= "<table width=\"100%\"><tr><td style=\"padding:1px; background-color:black;\"><table width=\"100%\" style=\"margins:0px;\" cellspacing=\"1\" cellpadding=\"3\"><tr><th colspan=\"4\" style=\"background-color:#dddddd;\"><center><a href=\"suggestions.php?do=new\">Create a New Thread</a></center></th></tr><tr><th width=\"44%\" style=\"background-color:#dddddd;\">Thread Title</th><th width=\"2%\" style=\"background-color:#dddddd;\">Replies</th><th  width=\"10%\" style=\"background-color:#dddddd;\">Author</th><th  width=\"30%\" style=\"background-color:#dddddd;\">Last Post</th></tr>\n";
	$count = 1;
    if (mysql_num_rows($query) == 0) {
       $page .= "<tr><td style='background-color:#ffffff;' colspan='4'><b>No threads in Suggestions forum.</b></td></tr>\n";
    } else {
      while ($row = mysql_fetch_array($query)) {
		...
		if ($count == 1) {
                $page .= "<tr><td style=\"background-color:#ffffff;\">".$namelink."<a href=\"suggestions.php?do=thread:".$row["id"].":0\">".$row["title"]."</a></td><td style=\"background-color:#ffffff;\">".$row["replies"]."</td><td style=\"background-color:#ffffff;\">".$row["author"]."</td><td style=\"background-color:#ffffff;\">".$row["newpostdate"]."</td></tr>\n";
			$count = 2;
		} else {
                $page .= "<tr><td style=\"background-color:#eeeeee;\">".$namelink."<a href=\"suggestions.php?do=thread:".$row["id"].":0\">".$row["title"]."</a></td><td style=\"background-color:#eeeeee;\">".$row["replies"]."</td><td style=\"background-color:#eeeeee;\">".$row["author"]."</td><td style=\"background-color:#eeeeee;\">".$row["newpostdate"]."</td></tr>\n";
			$count = 1;
		}
	  }
    }
	...
	for($pagenum = 0; $pagenum <= $numpages; $pagenum++) {
		...
		$pagelink = $pagenum + 1;
		if ($start != $pagestart) {
		$page .= "<a href='suggestions.php?do=list:".$pagestart."'>".$pagelink."</a>   ";}
		else {
		$page .= "<i>".$pagelink."</i>   ";}
	}
	$page .= " ]</center></td></tr>";
    ...
    display($page, "Suggestions Forum");
...
global $controlrow, $userrow; 
    ...
    $query = doquery("SELECT * FROM {{table}} WHERE id='$id' OR parent='$id' ORDER BY id LIMIT $start,50", "suggestions");
    $query2 = doquery("SELECT title FROM {{table}} WHERE id='$id' LIMIT 1", "suggestions");
    $row2 = mysql_fetch_array($query2);
    
 $page = "<table width='100%' border='1'><tr><td class='title'>Suggestions Forum - View Thread</td></tr></table><p>[<a href=\"#bottom\">Go to Bottom</a>]<p>";
    $page .= "<table width=\"100%\"><tr><td style=\"padding:1px; background-color:black;\"><table width=\"100%\" style=\"margins:0px;\" cellspacing=\"1\" cellpadding=\"3\"><tr><td colspan=\"2\" style=\"background-color:#dddddd;\"><b><a href=\"suggestions.php\">Suggestions Forum</a> :: ".$row2["title"]."</b></td></tr>\n";
    $count = 1;
...
if ($row["close"] == 1)  {
 $page .= "<a name=\"bottom\"></a>[<a href=\"#top\">Go to Top</a>]<p><center><img src=\"img/padlock.gif\"><br><b>This thread has been Closed</b></center><p>";
    ...
    $page .= "<a name=\"bottom\"></a>[<a href=\"#top\">Go to Top</a>]<p><table width=\"100%\"><tr><td><b>Reply To This Thread:</b><br /><form action=\"suggestions.php?do=reply\" method=\"post\"><input type=\"hidden\" name=\"parent\" value=\"$id\" /><input type=\"hidden\" name=\"title\" value=\"Re: ".$row2["title"]."\" /><textarea name=\"content\" rows=\"7\" cols=\"40\"></textarea><br /><input type=\"submit\" name=\"submit\" value=\"Submit\" /> <input type=\"reset\" name=\"reset\" value=\"Reset\" /></form></td></tr></table>";

}

$page .= "You may return to the <a href=\"suggestions.php\">suggestions forum</a> main page, or use the compass on the right to start exploring.<br />\n";      
...
"Your account has been muted from the Player Chat, posting Comments, the Forum and from Game Mails.<p>This is most probably temporary due to you breaking the rules, or causing problems.<p>Please return to what you were previously doing.");
	...
	$query = doquery("INSERT INTO {{table}} SET id='',postdate=NOW(),newpostdate=NOW(),author='".$userrow["charname"]."',parent='$parent',replies='0',title='$title',content='$content'", "suggestions");
	$query2 = doquery("UPDATE {{table}} SET newpostdate=NOW(),replies=replies+1 WHERE id='$parent' LIMIT 1", "suggestions");
        $query = doquery("UPDATE {{table}} SET postcount=postcount+1 WHERE id='".$userrow["id"]."' LIMIT 1", "users");
	header("Location: suggestions.php?do=thread:$parent:0");
	die();
...
"Your account has been muted from the Player Chat, posting Comments, the Forum and from Game Mails.<p>This is most probably temporary due to you breaking the rules, or causing problems.<p>Please return to what you were previously doing.");
    ...
    if (isset($_POST["submit"])) {
        ...
        $query = doquery("INSERT INTO {{table}} SET id='',postdate=NOW(),newpostdate=NOW(),author='".$userrow["charname"]."',parent='0',replies='0',title='$title',content='$content'", "suggestions");
        $query = doquery("UPDATE {{table}} SET postcount=postcount+1 WHERE id='".$userrow["id"]."' LIMIT 1", "users");
         header("Location: suggestions.php");
        die();
    }
     $page = "<table width='100%' border='1'><tr><td class='title'>Suggestions Forum - Create Thread</td></tr></table><p>";
    $page .= "<table width=\"100%\"><tr><td><b>Create a New Thread:</b><br /><br/ ><form action=\"suggestions.php?do=new\" method=\"post\">Title:<br /><input type=\"text\" name=\"title\" size=\"50\" maxlength=\"50\" /><br /><br />Message:<br /><textarea name=\"content\" rows=\"7\" cols=\"40\"></textarea><br /><br /><input type=\"submit\" name=\"submit\" value=\"Submit\" /> <input type=\"reset\" name=\"reset\" value=\"Reset\" /></form></td></tr></table>";
$page .= "You may return to the <a href=\"suggestions.php\">suggestions forum</a> main page, or use the compass on the right to start exploring.<br />\n";      
...
function editpost($id) {
 global $userrow;
    ...
    if (isset($_POST["submit"])) {
        ...
        $errors = 0;
        $errorlist = "";
        if ($content == "") { $errors++; $errorlist .= "Content is required, return to the <a href=\"suggestions.php\">Support Forum</a>.<br />"; }
       if ($title == "") { $errors++; $errorlist .= "Title is required, return to the <a href=\"suggestions.php\">Support Forum</a>. If you wish to delete your whole Post, simply add a small comment saying you have removed it.<br />"; }


        
        if ($errors == 0) { 
            $query = doquery("UPDATE {{table}} SET title='$title', content='$content' WHERE id='$id' LIMIT 1", "suggestions");
            display("Your Post was successfully updated. Return to the <a href=\"suggestions.php\">Suggestions Forum</a>.","Edit Post");
        } else {
            display("<b>Errors:</b><br /><div style=\"color:red;\">$errorlist</div><br />Please go back and try again.", "Edit Post");
        }        
        
    }   
$idquery = doquery("SELECT * FROM {{table}} WHERE id='$id' LIMIT 1", "suggestions");
	$idrow = mysql_fetch_array($idquery);
	if ($idrow["author"] != $userrow["charname"]) {
        $page .= "<table width='100%' border='1'><tr><td class='title'>Suggestions Forum - Edit Denied</td></tr></table><p>";
	$page .= "You cannot edit this Post! This Post doesn't belong to you. Return to the <a href='suggestions.php'>Suggestions Forum</a>.<br>";
...
<form action="suggestions.php?do=editpost:$id" method="post">
<table width="90%">
<tr><td width="20%">Author:</td><td>{{author}} - <a href="suggestions.php?do=delete:$id">Delete Permanently</a></td></tr>
...
Return to the <a href="suggestions.php">Suggestions Forum</a>
END;
...
function delete($id) {
	...
	if ($idrow["author"] != $userrow["charname"]) {
        $page .= "<table width='100%' border='1'><tr><td class='title'>suggestions Forum - Edit Denied</td></tr></table><p>";
	$page .= "You cannot delete this Post! This Post doesn't belong to you. Return to the <a href='suggestions.php'>Suggestions Forum</a>.<br>";
	display($page, "Delete Post");
	} 
	    $query = doquery("SELECT * FROM {{table}} WHERE id='$id' LIMIT 1", "suggestions");
    $row = mysql_fetch_array($query);

	$query = doquery("DELETE FROM {{table}} WHERE id='$id' LIMIT 1", "suggestions");
	$query = doquery("UPDATE {{table}} SET postcount=postcount-1 WHERE id='".$userrow["id"]."' LIMIT 1", "users");
	header("Location: suggestions.php");
	die();
